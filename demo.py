...
# Check if the haarcascade XML file is loaded correctly
if face_cascade.empty():
    print("Error: Haarcascade XML file not loaded correctly.")
    exit()
...
while True:
    ...
    for (x, y, w, h) in faces:
        ...
        img_model = np.reshape(img_gray, (
        1, 96, 96, 1))  # Model takes input of shape = [batch_size, height, width, no. of channels]
        ...
        img_copy = cv2.cvtColor(img_copy,
                                cv2.COLOR_BGR2BGRA)  # Used for transparency overlay of filter using the alpha channel
            ...
            for i in range(0, sw):  # Overlay the filter based on the alpha channel
                for j in range(0, sh):
                    if santa_filter[i, j][3] != 0:
                        ...
                        if 0 <= top_lip_coords[1] + i + y - 20 < img_copy.shape[0] and 0 <= left_lip_coords[
                            0] + j + x - 60 < img_copy.shape[1]:
                            img_copy[top_lip_coords[1] + i + y - 20, left_lip_coords[0] + j + x - 60] = santa_filter[
                                i, j]
        else:
        ...
        if hat is not None:
            ...
            for i in range(0, hw):  # Overlay the filter based on the alpha channel
                for j in range(0, hh):
                    if hat[i, j][3] != 0:
                        # Ensure the coordinates are within the bounds
                        if 0 <= i + y - brow_coords[1] * 2 < img_copy.shape[0] and 0 <= j + x - left_eye_coords[
                            0] * 1 + 20 < img_copy.shape[1]:
        ...
        if glasses is not None:
            ...
            for i in range(0, gw):  # Overlay the filter based on the alpha channel
                for j in range(0, gh):
                    if glasses[i, j][3] != 0:
                        # Ensure the coordinates are within the bounds
                        if 0 <= brow_coords[1] + i + y - 50 < img_copy.shape[0] and 0 <= left_eye_coords[
                            0] + j + x - 60 < img_copy.shape[1]:
