...
def has_nan(keypoints):
    return any(math.isnan(x) for x in keypoints)

# Function which plots an image with its corresponding keypoints
...
for i in range(len(training)):
    ...
    if not has_nan(points):
        ...
        keypoints = training.iloc[i, :-1].astype(float).values
...
def augment(img, points):
    ...
    for i in range(0, len(f_points), 2):  # Mirror the key point coordinates
        x_renorm = (f_points[i] + 0.5) * 96  # Denormalize x-coordinate
        dx = x_renorm - 48  # Get distance to midpoint
        x_renorm_flipped = x_renorm - 2 * dx
        f_points[i] = x_renorm_flipped / 96 - 0.5  # Normalize x-coordinate
...
def get_test_data():
    imgs_test_reshaped = np.reshape(imgs_test, (imgs_test.shape[0], imgs_test.shape[1], imgs_test.shape[2], 1))
    return imgs_test_reshaped
